...
void CentroidTracker::Init(cv::Mat frame, unique_ptr<BaseDetector> detector, NetworkReid* reid)
{
...
//        }
        ...
        //reid
        if(tracker->isReid == false && reid != nullptr)
        {
            unsigned time = QDateTime::currentDateTime().toTime_t();
            Mat roi = Mat(*img, tracker->newestRect);
            QImage qRoi = MatImageToQt(roi);
            reid->SendQImage2ReidModel(time, &qRoi);
            tracker->isReid = true;
        }
